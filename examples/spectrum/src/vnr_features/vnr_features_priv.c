...
#include "mel_filter_1024_64_compact.h"

#define mel_filter_compact_start_bins   mel_filter_1024_64_compact_start_bins
#define mel_filter_compact_q31          mel_filter_1024_64_compact_q31
...
void vnr_priv_mel_compute(float_s32_t *filter_output, const bfp_complex_s32_t *X) {
    ...
    for(unsigned i=0; i<num_even_filters; i++) {
        ...
        unsigned filter_start = mel_filter_compact_start_bins[2*i];
        unsigned filter_length = mel_filter_compact_start_bins[2*(i+1)] - filter_start;
        ...
        bfp_s32_init(&filter_subset, (int32_t *) &mel_filter_compact_q31[filter_start], filter_exp, filter_length, 0);
    ...
    bfp_s32_t filter;
    bfp_s32_init(&filter, (int32_t *) &mel_filter_compact_q31[0], filter_exp, AUDIO_FEATURES_NUM_BINS, 0);
    ...
    for(unsigned i=0; i<num_odd_filters; i++) {
        unsigned filter_start = mel_filter_compact_start_bins[(2*i) + 1];
        unsigned filter_length = mel_filter_compact_start_bins[(2*(i+1)) + 1] - filter_start;
        // Create input spectrum subset BFP structure
        bfp_s32_t spect_subset;
